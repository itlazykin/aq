...
Strangler Fig Pattern — постепенно заменяю функциональность монолита новыми автономными сервисами. Например, выношу платёжную систему в отдельный сервис и перенаправляю трафик через API Gateway. Монолит продолжает работать,в него просто не добавляют новые фичи а пилят их вмикросервисах.
...
Каждый микросервис — это отдельный модуль, который решает одну конкретную задачу. Это позволяет команде работать над разными сервисами параллельно, независимо друг от друга.
...
CAP говорит:
При сетевом сбое (P) ты можешь выбрать только C или A, но не оба сразу

PACELC:
При Разделении — выбери между Доступностью и Согласованностью,
Иначе — выбери между Задержкой и Согласованностью.
